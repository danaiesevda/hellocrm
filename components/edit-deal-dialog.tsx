...
export function EditDealDialog({
  ...
  useEffect(() => {
    if (deal) {
      // Format closeDate for input (YYYY-MM-DD)
  ...
  const onSubmit = async (data: DealFormValues) => {
    ...
    try {
      ...
      const errorMessage = error?.message || "Failed to update deal. Please try again."
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`/api/data?type=deal&id=${deal.id}`, {
        method: "DELETE",
      })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!bg-crm-surface !border-crm-border max-w-2xl" style={{ backgroundColor: 'var(--color-crm-surface)', opacity: 1 }}>
        <DialogHeader>
          <DialogTitle className="text-crm-text-primary">Edit Deal</DialogTitle>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="!bg-crm-surface !border-crm-border">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-crm-text-primary">Delete Deal</AlertDialogTitle>
